...
class ProbabilisticModel(metaclass = ABCMeta):
    ...
    def __init__(self, parameters):
        ...
        # no clue for why this is needed:
        self.updated = False #whether the node has been touched


    def _unvisit(self):
        for p in self.parents:
            if(isinstance(p, ProbabilisticModel)):
                p._unvisit()
        self.visited = False
    ...
    def fix_parameters(self, parameters=None, rng=np.random.RandomState()):
        ...
        self._unvisit()
        fix_check = self._fix_parameters(parameters=parameters, rng=rng)
        self._unvisit()
        return fix_check


    def _fix_parameters(self, parameters=None, rng=np.random.RandomState()):
        """
        This is the private, recursive version of the function fix_parameters.
        """

        parameter_values_temp = []
        if(not(parameters)):
            # use self.visted flag to recognize whether _fix_parameters has already been called on this instance
            if self.visited == False:
                for i in range(len(self.parents)):
                    if(isinstance(self.parents[i], ProbabilisticModel)):
                        self.parents[i]._fix_parameters()
                ...
                print('Parameters already fixed for %s to %s' % (self.__str__(), str(self.parameter_values)) )

        else:
            # let's omit this part for the moment
...
class Uniform(ProbabilisticModel):
        ...
        for i in range(2):
            for j in range(len(parameters[i])):
                ...
                if(isinstance(parameters[i][j], ProbabilisticModel)):
                    num_parameters += parameters[i][j].dimension()
                else:
                    num_parameters += 1

        self._num_parameters = num_parameters
        self._dimension = int(num_parameters / 2)
    ...
    def _check_parameters(self, parameters):
        # NOTE: only partially refactored
    ...
    # def fix_parameters(self, parameters=None, rng=np.random.RandomState()):
    #     print('Fixing parameters for %s to %s' % (self.__str__(), parameters.__str__()))
    #     if (not(parameters)):
